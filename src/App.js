...
function FormSplittBill({ selectedFriend, onSplitBill }) {
  const [bill, setBill] = useState("");
  const [paidUser, setPaidUser] = useState("");
  const paidByFriend = bill ? bill - paidUser : "";
  const [whoIsUser, setWhoIsUser] = useState("user");
  function handleSubmit(e) {
    e.preventDefault();
    if (!bill || !paidUser) return;
    onSplitBill(whoIsUser === "user" ? paidByFriend : -paidUser);
  }
  return (
    <form className="form-split-bill" onSubmit={handleSubmit}>
      ...
      <select value={whoIsUser} onChange={(e) => setWhoIsUser(e.target.value)}>
